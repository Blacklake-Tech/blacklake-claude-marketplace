...
import { CloseCircleFilled, UploadOutlined, InboxOutlined, PictureOutlined } from '@ant-design/icons';
...
import { mockFields, mockSubObjects } from '../../Metadata';
import { convertMomentToString, convertMomentToTimestamp, FIELD_TYPE_DATE } from './utils';
import '../../components/styles/DataFormLayout.less';
import '../../components/styles/DetailLayoutContent.less';

const DEFAULT_OBJECT_CODE = 'cust_object344__c';
...
const FieldInput: React.FC<{ field: FieldDTO; disabled?: boolean; value?: any; onChange?: (value: any) => void }> = ({ field, disabled, value, onChange }) => {
  ...
  const handleFileChange = (fileList: UploadFile[]) => {
    // 对于附件和图片，value 应该是文件 ID 数组或文件对象数组
    // 这里先简单处理，实际应该根据后端接口返回的文件信息来设置
    ...
    case 14: // 附件
      return (
        <Upload
          ...
          beforeUpload={() => false} // 阻止自动上传，实际项目中需要实现上传逻辑
          multiple
        >
...
const SubObjectTable: React.FC<{ form: FormInstance; subObject: SubObjectDTO; namePath: (string | number)[] }> = ({ form, subObject, namePath }) => {
  ...
  const handleDelete = (idx: number) => {
    ...
    updateFormValue(tableData.filter((_: any, i: number) => i !== realIdx));
  };
    ...
    ...(subObject.fieldList || [])
      ...
      .map((f) => ({
        ...
        render: (_: any, record: any, index: number) => {
          ...
          return <SubObjectFieldInput field={f} fieldNamePath={[...namePath, realIdx, f.fieldCode]} form={form} isReadOnly={false} onFieldChange={markFieldTouched} />;
        },
      })),
...
export default () => {
  ...
  useEffect(() => {
    ...
    console.log('[create] 元数据初始化完成，字段数:', mockFields.length, '从对象数:', mockSubObjects.length);
  }, []);
  ...
  const createFieldItem = (field: FieldDTO) => {
    ...
    if (field.fieldType === FIELD_TYPE_DATE) {
      item.getValueProps = (v: any) => ({ value: v ? moment(v) : undefined });
    ...
    try {
      ...
      fields.forEach((f) => {
        if (!f.isRefer && values[f.fieldCode] !== undefined) {
          // 日期字段转换为时间戳（毫秒）
          if (f.fieldType === FIELD_TYPE_DATE) {
            mainFields[f.fieldCode] = convertMomentToTimestamp(values[f.fieldCode]);
          } else {
            mainFields[f.fieldCode] = convertMomentToString(values[f.fieldCode]) ?? values[f.fieldCode];
          }
        }
      });
      
      // 构建 records 数组，每个记录包含一个 fields 对象
      // 根据 API 文档，records 是必填的，至少需要一个记录
      const records = [{
        fields: mainFields,
      }];
      
      // 处理从对象 - 转换为新格式
      const sonObjects: Array<{
        objectCode: string;
        records: Array<{
      ...
      values.sonObjects?.forEach((so: any) => {
        ...
        const sonRecords: Array<{ fields: Record<string, any> }> = [];
        
        so.instances.forEach((inst: any) => {
          const sonFields: Record<string, any> = {};
          subObj?.fieldList.forEach((f) => {
            if (!f.isRefer && inst[f.fieldCode] !== undefined) {
              // 日期字段转换为时间戳（毫秒）
              if (f.fieldType === FIELD_TYPE_DATE) {
                sonFields[f.fieldCode] = convertMomentToTimestamp(inst[f.fieldCode]);
              } else {
                sonFields[f.fieldCode] = convertMomentToString(inst[f.fieldCode]) ?? inst[f.fieldCode];
              }
            }
          });
          
          if (Object.keys(sonFields).length > 0) {
            sonRecords.push({ fields: sonFields });
          }
        });
        
        if (sonRecords.length > 0) {
          sonObjects.push({
      ...
      // 构建请求数据
      const submitData = {
        fields: mainFields, // 自定义对象字段列表（object）
        objectCode: DEFAULT_OBJECT_CODE,
        records: records, // 自定义对象字段列表（array of objects）
        ...(sonObjects.length > 0 && { sonObjects }), // 可选：从对象列表
        sync: false, // 可选：是否同步刷新数据
      };
      
      const response = await customObjectApi.createNew(submitData);
      ...
      message.error(error?.errorFields ? '请检查表单填写是否正确' : error?.message || '创建失败');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="bl-dataFormLayout" style={{ height: '100%' }}>
      <Spin spinning={loading}>
        <div className="data-layout-context">
          {`新建${DEFAULT_OBJECT_NAME}` && (
            <div className="data-layout-title">
              <Row justify={'space-between'}>
                <Col span={14}>
                  <h2 className="bl-layout-title-text">{`新建${DEFAULT_OBJECT_NAME}`}</h2>
                </Col>
              </Row>
            </div>
          )}
