...
│   ├── normalize-commits.md  # 规范化提交历史命令（v2.0）
│   └── quick-commit.md       # 智能快速提交命令（v2.0）
├── skills/                   # 技能定义（新增 v2.0）
│   └── git-workflow/        # Git 工作流技能
│       ├── SKILL.md         # 技能定义（500+ 行）
│       ├── COMMON.md        # Git 通用规范（300+ 行）
│       └── GIT_COMMON.md    # 符号链接到 COMMON.md
└── README.md                # 本文件
```

**注意**：`common/` 目录已移除，Git 相关规范现在统一在 `skills/git-workflow/COMMON.md` 中。未来如果添加其他能力（如代码审查、文档生成等），可以创建对应的 skill 目录。
