#!/bin/bash
# Continuous Learning v2 - 观察 Hook
#
# 捕获工具使用事件用于模式分析。
# Claude Code 通过 stdin 以 JSON 格式传递 hook 数据。
#
# Hook 配置（在 ~/.claude/settings.json 中）：
#
...
# 从 stdin 读取 JSON（Claude Code hook 格式）
INPUT_JSON=$(cat)
...
# 使用 python 解析（比 jq 更可靠处理复杂 JSON）
PARSED=$(python3 << EOF
import json
import sys
...
try:
    ...
    # 提取字段 - Claude Code hook 格式
    hook_type = data.get('hook_type', 'unknown')  # PreToolUse 或 PostToolUse
