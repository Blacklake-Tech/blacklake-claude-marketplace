...
1. **检查参数**：
   - 如果提供了 `$ARGUMENTS`（自定义消息）：
     ...
     - 不符合则提示用户修正并退出
   - 如果未提供参数：继续自动生成流程
   - **特殊处理：如果参数包含"升级版本"或"version"关键词** → 执行【版本升级流程】

---

### 【版本升级流程】

用户请求：`/quick-commit 升级版本`、`/quick-commit 升级到 x.x.x`

#### 1. 查找版本文件

**优先级**：
1. **CLAUDE.md 明确指定** → 使用指定文件
2. **常见版本文件自动推测**：pyproject.toml → package.json → pom.xml → VERSION
3. **多个版本文件时** → 让用户选择

#### 2. 升级版本号

- **默认**：最小版本 +1（如 1.3.0 → 1.3.1）
- **指定版本**：如 `升级到 2.0.0` → 直接使用 2.0.0
- 更新文件后自动暂存

#### 3. 提交消息

- 自动生成：`release: 升级到 {新版本号}`
- 跳过后续自动推断步骤
...
| 场景 | 处理 |
|-----|------|
| 未找到版本文件 | 提示用户在 CLAUDE.md 中添加版本文件位置 |
| 多个版本文件 | 让用户选择 |
