...
SKILL_TEMPLATE = """---
name: {skill_name}
description: [待办：完整且信息丰富的技能功能说明以及何时使用。包括何时使用此技能 - 触发它的特定场景、文件类型或任务。]
---
...
**2. 基于任务**（最适合工具集合）
- 当技能提供不同的操作/功能时效果很好
- 示例：PDF 技能，包含"快速入门" → "合并 PDF" → "拆分 PDF" → "提取文本"
...
完成后删除整个"组织此技能"部分 - 这只是指导。]

## [待办：根据所选结构替换为第一个主要部分]

[待办：在此添加内容。查看现有技能中的示例：
- 技术技能的代码示例
- 复杂工作流程的决策树
- 具有实际用户请求的具体示例
- 根据需要引用脚本/模板/参考资料]
...
**注意：** 脚本可能在不加载到上下文中的情况下执行，但仍可被 Claude 读取以进行修补或环境调整。

### references/
旨在加载到上下文中以指导 Claude 的流程和思考的文档和参考材料。
