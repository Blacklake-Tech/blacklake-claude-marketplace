...
- **造数据场景**：列出4步流程的任务清单（参考 es-log skill 的【造数据工作流程】）
...
1. **日志查询**：根据条件查询接口日志、外部接口日志、事件日志、中间表日志
   - 详细工作流程请参考 es-log skill 的【查询操作】章节

2. **数据造数**：从生产环境查询数据，安全写入测试环境（Feature/Test）
   - 详细工作流程请参考 es-log skill 的【造数据工作流程】章节
   - 必须按顺序执行4个步骤，不得跳过

3. **时间修改**：修改已写入数据的执行时间，触发轮询通知
   - 详细工作流程请参考 es-log skill 的【修改已写入数据的执行时间】章节
...
**工作流程**：参考 es-log skill 的【查询操作】章节，包括理解查询条件、构建查询、执行查询、展示结果。
...
详细步骤说明请参考 es-log skill 的【造数据工作流程】章节。
...
**工作流程**：参考 es-log skill 的【修改已写入数据的执行时间】章节。
...
- 如果跳过此步骤，视为严重错误
- 使用 es-log skill 的【造数据工作流程】中的步骤 3 检查方法
...
6. **参考 Skill**：所有操作模板和工作流程参考 es-log skill
