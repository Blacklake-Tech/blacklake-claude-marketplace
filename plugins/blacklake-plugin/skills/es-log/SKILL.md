...
请确认目标 orgId 后，我将执行写入操作。
```

### 步骤 3：转化成对应的目标索引并写入数据

**⚠️ 写入前必须检查**：确认目标索引不包含生产环境标识（`v3master`、`hwv3master`），**严令禁止写入生产环境**！

**⚠️ 重要**：**必须先执行步骤 3.1 检查 ID 冲突**，只有在确认不存在冲突或用户明确确认覆盖后，才能执行写入操作。
...
### 步骤 3.1：检查目标索引是否存在相同文档ID（必须执行）

**⚠️ 这是必须步骤，不能跳过！** 在写入数据前，必须先检查目标索引模式中是否已存在相同文档ID的数据。
...
**执行流程**：
1. **必须先执行步骤 3.1 的检查**：查询目标索引模式中是否存在相同文档ID
2. **如果不存在**：直接执行写入操作
3. **如果存在**：展示完整 JSON，询问用户是否覆盖，等待用户明确确认（"确认覆盖"）后才能执行写入操作
4. **禁止跳过检查步骤**：无论是否存在冲突，都必须先执行检查

**只有在步骤 3.1 检查完成（不存在冲突或用户明确确认覆盖）后，才能执行以下写入操作**。
...
4. **ID 冲突检查（必须）**：
   ...
   - **写入前必须检查**：使用通配符索引模式查询目标环境是否存在相同文档ID的数据（因为相同文档ID可能存在于不同日期的索引中）
   - **如果存在**：必须直接展示完整的原始 JSON 数据（现有数据和即将写入的数据），明确提醒用户，并询问是否覆盖
   - **等待用户明确确认**（"确认覆盖"或"取消"）后才能执行写入操作
   - 禁止在未确认的情况下直接覆盖数据
